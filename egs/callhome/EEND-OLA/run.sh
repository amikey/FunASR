...
simu_feats_dir="/nfs/wangjiaming.wjm/EEND_ARK_DATA/data/simu_data" #feature output dictionary
simu_train_dataset=train
simu_valid_dataset=dev

# model average
average_2spkr_start=91
average_2spkr_end=100
...
simu_2spkr_ave_id=avg${average_2spkr_start}-${average_2spkr_end}
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    echo "averaging model parameters into $simu_2spkr_model_dir/$simu_2spkr_ave_id.pb"
    models=`eval echo ${exp_dir}/exp/${simu_2spkr_model_dir}/{$average_2spkr_start..$average_2spkr_end}epoch.pb`
