...
class AudioMultiTaskDataset(IterableDataset):
        ...
        for index in worker_data_index:
            ...
            for data_file, data_type in zip(data_file_list, data_type_list):
                if data_type == "kaldi_ark":
                    ...
                    lid_reader = open(data_file, "r")
                    reader_list.append(lid_reader)
                elif data_type == "none":
                    continue
                else:
                    raise TypeError("Data type {} is not supported".format(data_type))
            ...
            for items in zip(*reader_list):
                ...
                for item, (data_name, data_type) in zip(items, zip(data_name_list, data_type_list)):
                    if data_type == "kaldi_ark":
                        ...
                        lid = item
                        lid_labels = lid.strip().split()
                        sample_dict[data_name] = lid_labels[1:]
                        if "key" not in sample_dict:
                            sample_dict["key"] = lid_labels[0]
                    else:
                        text = item
                        segs = text.strip().split()
                        sample_dict[data_name] = segs[1:]
...
def MultitaskDataset(data_list_file,
            dict,
            seg_dict,
            punc_dict,
            bpe_tokenizer,
    ...
    scp_lists = read_lists(data_list_file)
    shuffle = conf.get('shuffle', True)
    data_names = conf.get("data_names", "speech,text,lid")
    data_types = conf.get("data_types", "kaldi_ark,text,lid")
    dataset = AudioMultiTaskDataset(scp_lists, data_names, data_types, frontend_conf=frontend_conf, shuffle=shuffle, mode=mode)
