...
class MultiHeadedAttentionSANMLoRA(nn.Module):
    """Multi-Head Attention layer.
    ...
    def __init__(self, n_head, in_feat, n_feat, dropout_rate, kernel_size, sanm_shfit=0, lora_list=None, lora_rank=8):
        """Construct an MultiHeadedAttention object."""
        super(MultiHeadedAttentionSANMLoRA, self).__init__()
        assert n_feat % n_head == 0
        # We assume d_v always equals d_k
        self.d_k = n_feat // n_head
        self.h = n_head
        if lora_list is not None:
            ...
            self.linear_q_k_v = lora.MergedLinear(in_feat, n_feat * 3, r=lora_rank, enable_lora=lora_qkv_list)
        else:
            self.linear_out = nn.Linear(n_feat, n_feat)
            self.linear_q_k_v = nn.Linear(in_feat, n_feat * 3)
