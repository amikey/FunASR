...
from funasr_onnx.abs_asr_model import AbsASRModel
from funasr_onnx.beam_search.beam_search import Hypothesis
...
class Speech2Text(AbsASRModel):
    ...
    def __init__(
        ...
        self._check_argument(model_dir)
        self._load_config()
    ...
    def __call__(
        ...
        # lengths: (1,)
        lengths = np.array([speech.shape[0]]).astype(np.int64)
        ...
        #print("encode shape {}".format(enc.shape))

        #tmp_enc = enc[0]
        #print("tmp_enc {0}".format(tmp_enc.shape))
        print("encoder out shape {}".format(enc.shape))
