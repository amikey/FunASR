...
from funasr_onnx.beam_search.hyps import Hypothesis
from funasr_onnx.scorer.interface import (PartialScorerInterface,
                                              ScorerInterface)
from funasr_onnx.utils.config import Config
from funasr_onnx.utils.function import end_detect, topk
...
class BeamSearch:
    ...
    def score_full(
        ...
        for k, d in self.full_scorers.items():
            print("score: {0} {1}".format(k, d))
            logp, states[k] = d.score(hyp.yseq, hyp.states[k], x)
            scores[k] = np.squeeze(logp, axis=0)
            print("scores[k] shape {}".format(scores[k].shape))
            print("states[k] len {0}".format(len(states[k])))
            tmp_score = states[k][0]
            print("type {}".format(type(tmp_score)))
    ...
    def score_partial(
        ...
        for k, d in self.part_scorers.items():
            print("partial score".format(k))
            print("partial score ids {}".format(ids))
