...
class Paraformer_dashscope():
    def __init__(self, model_dir: Union[str, Path] = None,
        ...
        encoder_model_file = os.path.join(model_dir, 'encoder.torchscripts')
        decoder_model_file = os.path.join(model_dir, 'decoder.torchscripts')
        if quantize:
            encoder_model_file = os.path.join(model_dir, 'encoder_quant.torchscripts')
            decoder_model_file = os.path.join(model_dir, 'decoder_quant.torchscripts')
        ...
        self.ort_encoder_infer = torch.jit.load(encoder_model_file)
        self.ort_decoder_infer = torch.jit.load(decoder_model_file)
    ...
    def prepare_cache(self):
        cache = {"encoder_outputs": None, "alphas": None, "last_chunk": False,
                 "feats": np.zeros((self.chunk_size[0] + self.chunk_size[2], self.feats_dims)).astype(np.float32)}
        return cache
        ...
        for beg_idx in range(0, waveform_nums, self.batch_size):
            ...
            try:
                ...
                for i in range(max_chunk_num):
                    ...
                    feats_chunk = torch.from_numpy(feats_chunk).type(torch.float32)
                    feats_chunk_lens = np.array(feats_chunk_lens).astype(np.int32)
                    feats_chunk_lens = torch.from_numpy(feats_chunk_lens).type(torch.int32)
                    # forward encoder
                    with torch.no_grad():
                        if int(self.device_id) == -1:
                            enc, enc_lens, alphas = self.ort_encoder_infer(feats_chunk, feats_chunk_lens)
                        else:
                            enc, enc_lens, alphas = self.ort_encoder_infer(feats_chunk.cuda(), feats_chunk_lens.cuda())
                            enc, enc_lens, alphas = enc.cpu(), enc_lens.cpu(), alphas.cpu()
                    index = 0
                    for j in range(end_idx - beg_idx):
                        if i+1 <= cache[j]["chunk_num"]:
                ...
                # forward decoder
                enc = torch.from_numpy(enc).type(torch.float32)
                enc_lens = torch.from_numpy(enc_lens).type(torch.int32)
                pre_acoustic_embeds = torch.from_numpy(pre_acoustic_embeds).type(torch.float32)
                pre_token_length = torch.from_numpy(pre_token_length).type(torch.int32)
                with torch.no_grad():
                    if int(self.device_id) == -1:
                        am_scores, _ = \
                            self.ort_decoder_infer(enc, enc_lens, pre_acoustic_embeds, pre_token_length)
                    else:
                        am_scores, _ = \
                            self.ort_decoder_infer(enc.cuda(), enc_lens.cuda(), pre_acoustic_embeds.cuda(), pre_token_length.cuda())
                        am_scores = am_scores.cpu()
            except:
